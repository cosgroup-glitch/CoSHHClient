...
public class AlarmWindow extends Window {
	...
	public AlarmWindow() {
		...
		add(bottomNote = new Label("NOTE: You can add your own .wav alarm files in " + AlarmManager.alarmDir().getAbsolutePath(), new Text.Foundry(Text.sans, 12)), prev.pos("ul").adds(0, 34).x(UI.scale(80)));
		...
		add(new Button(UI.scale(100), "Add Alarm") {
			...
			public void resetBottomNote() {
				bottomNote.settext("NOTE: You can add your own .wav alarm files in " + AlarmManager.alarmDir().getAbsolutePath());
				bottomNote.setcolor(Color.WHITE);
				bottomNote.c.x = UI.scale(80);
				future.cancel(true);
			}
		...
		add(new Button(UI.scale(180), "Load Nightdawg's Defaults") {
			...
			public void click() {
				...
				if (ui != null && ui.gui != null)
					ui.gui.msg("Default alarms restored!", GameUI.MsgType.INFO);
				AlarmManager.load(al);
				AlarmManager.save();
...
//			};
			...
			prev = add(new Button(UI.scale(70), "Preview") {
				...
				public boolean mousedown(MouseDownEvent ev) {
					if(ev.b != 1)
						return true;
					File file = new File(AlarmManager.alarmDir(), getAlarmFilename());
					if(!file.exists() || file.isDirectory()) {
						if (ui != null && ui.gui != null)
							ui.gui.msg("Error while playing an alarm, file " + file.getAbsolutePath() + " does not exist!", GameUI.MsgType.ERROR);
						return super.mousedown(ev);
					}
					try {
                        ...
                        ui.sfx(new Audio.VolAdjust(klippi, getVolume()/50.0));
