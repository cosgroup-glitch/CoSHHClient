...
public class QuestWnd extends Widget {
	...
	public static class Box extends Widget implements Info, QView.QVInfo {
		...
		if(msg == "conds") {
		    ...
		    this.ui.gui.updateQuestObjectives(this);
		} else {
	...
	questbox = add(new Widget(new Coord(attrw, height)) {
		...
		if(w == quest) {
		    if(w instanceof Quest.Box) {
			GameUI gui = getparent(GameUI.class);
			if(gui != null)
			    gui.clearQuestObjectives(((Quest.Box)w).questid());
		    }
		    quest = null;
		}
	    }
	...
	if(place == "quest") {
	    ...
	    GameUI gui = getparent(GameUI.class);
	    if((gui != null) && (child instanceof Quest.Box))
		gui.showQuestObjectives((Quest.Box)child);
	} else {
